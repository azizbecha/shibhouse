...
import { mapPeersData } from '../lib/peerHelper'
...
export const PeerContextProvider = ({ children, initialContext }) => {
  ...
  async function updateRoom(id, data) {
    const washingtonRef = doc(fireStore, "rooms", id);
  ...
  useEffect(() => {
    ...
    const update = setInterval(async () => {
      ...
      if (docSnap.exists()) {
        // To verifiy that the room exists
        updateRoom(roomId, {
          users: (connectedPeersRef.current.length + 1),
